...
export default function TradeForm() {
  ...
  const cModelRef = useRef<HTMLSelectElement>(null)
  const cStorageRef = useRef<HTMLSelectElement>(null)
  const cCondRef = useRef<HTMLSelectElement>(null)
  const rModelRef = useRef<HTMLSelectElement>(null)
  const rStorageRef = useRef<HTMLSelectElement>(null)
  ...
  const handleSubmit = async () => {
    ...
    const cm = cModelRef.current?.value
    const cs = cStorageRef.current?.value
    const cc = cCondRef.current?.value
    const rm = rModelRef.current?.value
    const rs = rStorageRef.current?.value
    
    if (!name || !phone || !cm || !cs || !cc || !rm || !rs) {
      showToast('⚠️ يرجى تعبئة جميع الحقول', 'warn')
      return
    }
    ...
    try {
      ...
      if (files) {
        for (let i = 0; i < files.length; i++) {
          const u = await uploadImg(supabase, files[i], 'tr')
          if (u) mediaUrls.push(u)
        }
      }
      if (camFiles) {
        for (let i = 0; i < camFiles.length; i++) {
          const u = await uploadImg(supabase, camFiles[i], 'trc')
          if (u) mediaUrls.push(u)
        }
      }
  ...
  const resetForm = () => {
    ...
    if (cModelRef.current) cModelRef.current.value = ''
    if (cStorageRef.current) cStorageRef.current.value = ''
    if (cCondRef.current) cCondRef.current.value = ''
    if (rModelRef.current) rModelRef.current.value = ''
    if (rStorageRef.current) rStorageRef.current.value = ''
  ...
  return (
    <section id="trade" className="form-section">
      <h2 
      ...
      >
        ...
        <svg 
          ...
          style={{ transition: 'transform 0.35s ease', transform: tradeOpen ? 'rotate(180deg)' : 'rotate(0deg)', color: 'var(--neon-2)' }}
        >
          <path d="M6 9l6 6 6-6" />
        </svg>
      </h2>
      ...
      <div 
        ...
        style={{
          overflow: 'hidden', 
          maxHeight: tradeOpen ? '2000px' : '0', 
          transition: 'maxHeight 0.5s cubic-bezier(0.4,0,0.2,1), opacity 0.4s ease', 
          opacity: tradeOpen ? 1 : 0
        }}
      >
        {!success ? (
          <div className="form-card" id="t-card">
            ...
            <div className="field-wrap">
              ...
              <select ref={cModelRef} className="fi" required defaultValue="">
            ...
            <div className="form-row">
              <div className="field-wrap">
                ...
                <select ref={cStorageRef} className="fi" required defaultValue="">
              ...
              <div className="field-wrap">
                ...
                <select ref={cCondRef} className="fi" required defaultValue="">
            ...
            <div className="field-wrap">
              ...
              <select ref={rModelRef} className="fi" required defaultValue="">
                <option value="" disabled>اختر الجهاز المطلوب</option>
                {ALL_MODELS.map(m => <option key={m} value={m}>{m}</option>)}
              </select>
            </div>
            
            <div className="field-wrap">
              <label className="field-label">تخزين المطلوب</label>
              <select ref={rStorageRef} className="fi" required defaultValue="">
                <option value="" disabled>التخزين</option>
                <option>128GB</option>
                <option>256GB</option>
                <option>512GB</option>
            ...
            <div className="field-wrap">
              ...
              <textarea ref={notesRef} className="fi" placeholder="أي تفاصيل إضافية..."></textarea>
            </div>
            
            <div className="field-wrap">
              <label className="field-label">صور جهازك الحالي (اختياري)</label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '8px' }}>
                <label htmlFor="t-media" className="upload-btn" style={{ margin: 0 }}>🖼️ المعرض</label>
                <label htmlFor="t-camera" className="upload-btn" style={{ margin: 0, background: 'rgba(255,255,255,0.08)' }}>📸 الكاميرا</label>
              </div>
              <input type="file" id="t-media" ref={mediaRef} multiple accept="image/*" style={{ display: 'none' }} onChange={handleFileChange} />
              <input type="file" id="t-camera" ref={mediaCamRef} accept="image/*" capture="environment" style={{ display: 'none' }} onChange={handleFileChange} />
              
              <div className="upload-preview" id="t-preview">
                {previews.map((url, i) => (
                  <img key={i} src={url} className="upload-thumb" alt="Preview" />
                ))}
              </div>
              
              <div className="progress-bar-wrap" id="t-prog" style={{ display: showProgress ? 'block' : 'none' }}>
                <div className="progress-bar-fill" id="t-prog-fill" style={{ width: progressWidth }}></div>
              </div>
            </div>
          ...
          <div className="form-success" id="t-success" style={{ display: 'flex' }}>
